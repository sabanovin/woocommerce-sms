jQuery(document).ready(function () {
    ...
    if (jQuery().select2)
        jQuery('.hannanstd_woo_tabs_content_field').select2();

    jQuery('#add_another_sms_tab').on('click', function (e) {

        if (jQuery().select2)
            jQuery('.hannanstd_woo_tabs_content_field').select2("destroy");
        ...
        var new_count = parseInt(jQuery('#number_of_tabs_sms').val()) + parseInt(1);
        /* get new number of cloned element */
        ...
        clone_container.children('p').each(function () {
            ...
            jQuery('.new_duplicate_row_sms').first().before('<div class="hannanstd-woo-custom-tab-divider"></div>');

        });

        move_tab_content_buttons.clone().insertAfter(jQuery('.hannanstd-woo-custom-tab-divider').last()).addClass('last-button-holder-sms');
    ...
    */
    jQuery('body').on('click', '.number_of_tabs_sms', function (e) {
        ...
        var count_post_remove = parseInt(jQuery('#number_of_tabs_sms').val()) - parseInt(1);
        /* get new number of cloned element */
